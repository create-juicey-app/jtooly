...
export async function getServerSideProps({ req }) {
  ...
  const response = await fetch(`http://api.ipstack.com/${requestIp}?access_key=${apiKey}`);
...
export default function HomePage({ requestHeaders, requestIp, requestMethod, requestUserAgent, requestUrl, location }) {
  ...
  <Grid item xs={8}>
      ...
      {latitude && longitude && (
        ...
        <Grid item xs={8}>
      <Typography variant="caption">This is only clientside, nothing is sent to the server, i do not know any informations about this, this happens on your side. </Typography>
      </Grid> </Grid>
    </div>
  );
}
